...
import com.carledwinti.scala.oop.filesystem.files.{Directory, File}
...
class Echo(args: Array[String]) extends Command{
  ...
  def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String, append: Boolean): Directory = {
    /*
      if path is empty, then fail (currentDirecotry)
      else if no more things to explore = path.tail.isEmpty
        find the file tod create/add content to
        ...
        else replace entry with the NEW directory after, the recusive call
